...
import com.droi.volley.ResponseDelivery;
...
/**
 * Droi's custom implementation of the Google Volley RequestQueue.
 ...
 */
public class DroiRequestQueue extends RequestQueue {
    ...
    DroiRequestQueue(Cache cache, Network network, int threadPoolSize, ResponseDelivery delivery) {
        super(cache, network, threadPoolSize, delivery);
        mDelayedRequests = new HashMap<Request<?>, DelayedRequestHelper>(CAPACITY);
    }

    DroiRequestQueue(Cache cache, Network network, int threadPoolSize) {
        super(cache, network, threadPoolSize);
        mDelayedRequests = new HashMap<Request<?>, DelayedRequestHelper>(CAPACITY);
    }

    DroiRequestQueue(Cache cache, Network network) {
        super(cache, network);
        mDelayedRequests = new HashMap<Request<?>, DelayedRequestHelper>(CAPACITY);
    }
        ...
        DelayedRequestHelper(@NonNull final Request<?> request, int delayMs, @NonNull Handler handler) {
            ...
            mDelayedRunnable = new Runnable() {
                ...
                public void run() {
                    ...
                    DroiRequestQueue.this.add(request);
                }
            };
        }
