...
import com.droi.common.util.AsyncTasks;
...
import static com.droi.common.util.ResponseHeader.LOCATION;
...
public class MraidNativeCommandHandler {
    ...
    void createCalendarEvent(final Context context, final Map<String, String> params)
        ...
        if (isCalendarAvailable(context)) {
            try {
                ...
                DroiLog.d("no calendar app installed");
                throw new MraidCommandException(
                        "Action is unsupported on this device - no calendar app installed");
            } catch (IllegalArgumentException e) {
                DroiLog.d("create calendar: invalid parameters " + e.getMessage());
                throw new MraidCommandException(e);
            } catch (Exception e) {
                DroiLog.d("could not create calendar event");
                throw new MraidCommandException(e);
            }
        } else {
            DroiLog.d("unsupported action createCalendarEvent for devices pre-ICS");
    ...
    void storePicture(@NonNull final Context context,
        ...
        if (!isStorePictureSupported(context)) {
            DroiLog.d("Error downloading file - the device does not have an SD card mounted, or " +
        ...
        final DownloadImageAsyncTask downloadImageAsyncTask = new DownloadImageAsyncTask(context,
                new DownloadImageAsyncTask.DownloadImageAsyncTaskListener() {
                    ...
                    public void onSuccess() {
                        DroiLog.d("Image successfully saved.");
                    }

                    @Override
                    public void onFailure() {
                        Toast.makeText(context, "Image failed to download.", Toast.LENGTH_SHORT).show();
                        DroiLog.d("Error downloading and saving image file.");
    ...
    static class DownloadImageAsyncTask extends AsyncTask<String, Void, Boolean> {
        ...
        protected Boolean doInBackground(@NonNull String[] params) {
            ...
            try {
                final HttpURLConnection urlConnection =
                        DroiHttpUrlConnection.getHttpUrlConnection(uriString);
        ...
        private void loadPictureIntoGalleryApp(final String filename) {
            DroiMediaScannerConnectionClient mediaScannerConnectionClient =
                    new DroiMediaScannerConnectionClient(filename, null);
    ...
    private static class DroiMediaScannerConnectionClient
            implements MediaScannerConnection.MediaScannerConnectionClient {
        private final String mFilename;
        private final String mMimeType;
        private MediaScannerConnection mMediaScannerConnection;

        private DroiMediaScannerConnectionClient(String filename, String mimeType) {
            mFilename = filename;
            mMimeType = mimeType;
        }
