...
import com.droi.mobileads.VastErrorCode;
import com.droi.mobileads.VastMacroHelper;
import com.droi.mobileads.VastTracker;
    ...
    protected Response<Void> parseNetworkResponse(final NetworkResponse networkResponse) {
        if (networkResponse.statusCode != 200) {
            return Response.error(
                    new DroiNetworkError("Failed to log tracking request. Response code: "
                            + networkResponse.statusCode + " for url: " + getUrl(),
                            DroiNetworkError.Reason.TRACKING_FAILURE));
        }
        ...
        for (final String url : urls) {
            ...
            final TrackingRequest.Listener internalListener = new TrackingRequest.Listener() {
                ...
                public void onResponse(@NonNull String url) {
                    DroiLog.d("Successfully hit tracking endpoint: " + url);
                    if (listener != null) {
                ...
                public void onErrorResponse(final VolleyError volleyError) {
                    DroiLog.d("Failed to hit tracking endpoint: " + url);
                    if (listener != null) {
