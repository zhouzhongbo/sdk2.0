...
import static com.droi.common.UrlResolutionTask.UrlResolutionListener;
import static com.droi.network.TrackingRequest.makeTrackingHttpRequest;
...
public class UrlHandler {
    ...
    /**
     * {@code DroiSchemeListener} defines the methods that {@link UrlHandler} calls when handling
     * {@code HANDLE_DROI_SCHEME} URLs.
     */
    public interface DroiSchemeListener {
        void onFinishLoad();
        void onClose();
        void onFailLoad();
    }
    ...
    public static class Builder {
        ...
        @NonNull
        private DroiSchemeListener mDroiSchemeListener = EMPTY_DROI_SCHEME_LISTENER;
        private boolean skipShowDroiBrowser = false;
        ...
        /**
         * Sets the {@link DroiSchemeListener} for the {@code UrlHandler} to build.
         *
         * @param mDroiSchemeListener A {@code DroiSchemeListener} for the {@code UrlHandler}.
         * @return A {@link Builder} with the desired {@code DroiSchemeListener} added.
         */
        public Builder withDroiSchemeListener(
                @NonNull final DroiSchemeListener mDroiSchemeListener) {
            this.mDroiSchemeListener = mDroiSchemeListener;
            return this;
        }

        /**
         * If called, will avoid starting a {@link DroiBrowser} activity where applicable.
         * (see {@link Intents#showDroiBrowserForUrl(Context, Uri, String)})
         *
         * @return A {@link Builder} that will skip starting a {@code DroiBrowser}.
         */
        public Builder withoutDroiBrowser() {
            this.skipShowDroiBrowser = true;
            return this;
        }
        ...
        public UrlHandler build() {
            return new UrlHandler(supportedUrlActions, resultActions, mDroiSchemeListener,
                    skipShowDroiBrowser, creativeId);
        }
    }
    ...
    private static final DroiSchemeListener EMPTY_DROI_SCHEME_LISTENER =
            new DroiSchemeListener() {
    ...
    @NonNull
    private DroiSchemeListener mDroiSchemeListener;
    @Nullable private String mDspCreativeId;
    private boolean mSkipShowDroiBrowser;
    ...
    private UrlHandler(
            ...
            @NonNull final DroiSchemeListener droiSchemeListener,
            final boolean skipShowDroiBrowser,
            @Nullable final String dspCreativeId) {
        mSupportedUrlActions = EnumSet.copyOf(supportedUrlActions);
        mResultActions = resultActions;
        mDroiSchemeListener = droiSchemeListener;
        mSkipShowDroiBrowser = skipShowDroiBrowser;
    ...
    @NonNull
    DroiSchemeListener getDroiSchemeListener() {
        return mDroiSchemeListener;
    }

    boolean shouldSkipShowDroiBrowser() {
        return mSkipShowDroiBrowser;
    }
            ...
            if (urlAction.shouldTryHandlingUrl(destinationUri)) {
                try {
                    ...
                    if (!mAlreadySucceeded && !mTaskPending
                            ...
                            && !UrlAction.HANDLE_DROI_SCHEME.equals(urlAction)) {
