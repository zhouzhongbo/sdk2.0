...
import static com.droi.common.Constants.HTTP;
import static com.droi.common.Constants.HTTPS;
...
public enum UrlAction {
    ...
    /* 0 */ HANDLE_DROI_SCHEME(false) {
        @Override
        public boolean shouldTryHandlingUrl(@NonNull final Uri uri) {
            return "droi".equalsIgnoreCase(uri.getScheme());
        }
        ...
        protected void performAction(
            ...
            final UrlHandler.DroiSchemeListener mDroiSchemeListener =
                    urlHandler.getDroiSchemeListener();

            if ("finishLoad".equalsIgnoreCase(host)) {
                mDroiSchemeListener.onFinishLoad();
            } else if ("close".equalsIgnoreCase(host)) {
                mDroiSchemeListener.onClose();
            } else if ("failLoad".equalsIgnoreCase(host)) {
                mDroiSchemeListener.onFailLoad();
            } else {
                throw new IntentNotResolvableException("Could not handle Droi Scheme url: " + uri);
            }
        }
    },
    ...
    /* 1 */ IGNORE_ABOUT_SCHEME(false) {
        ...
        protected void performAction(
                ...
                @Nullable String creativeId)
                throws IntentNotResolvableException {
            DroiLog.d("Link to about page ignored.");
        }
    },
    ...
    /* 3 */ OPEN_NATIVE_BROWSER(true) {
        @Override
        public boolean shouldTryHandlingUrl(@NonNull final Uri uri) {
            return "droinativebrowser".equalsIgnoreCase(uri.getScheme());
        }
        ...
        protected void performAction(
                ...
                @Nullable String creativeId)
                throws IntentNotResolvableException {
            final String errorMessage = "Unable to load droi native browser url: " + uri;
            try {
    ...
    /* 5 */ OPEN_IN_APP_BROWSER(true) {
        ...
        protected void performAction(
                ...
                @Nullable String creativeId)
                throws IntentNotResolvableException {
            if (!urlHandler.shouldSkipShowDroiBrowser()) {
                Intents.showDroiBrowserForUrl(context, uri, creativeId);
            }
        }
    },
    ...
    /* 6 */ HANDLE_SHARE_TWEET(true) {
        ...
        public boolean shouldTryHandlingUrl(@NonNull final Uri uri) {
            ...
            return "droishare".equalsIgnoreCase(uri.getScheme())
    ...
    public void handleUrl(
            ...
            @Nullable String creativeId)
            throws IntentNotResolvableException {
        DroiLog.d("Ad event URL: " + destinationUri);
