...
import com.droi.common.DroiHttpUrlConnection;
...
import com.droi.common.util.Streams;
import com.droi.common.util.Strings;
...
import static com.droi.network.TrackingRequest.makeVastTrackingHttpRequest;
...
public class VastXmlManagerAggregator extends AsyncTask<String, Void, VastVideoConfig> {
    ...
    private static final String DROI = "Droi";
    ...
    protected VastVideoConfig doInBackground(@Nullable String... strings) {
        ...
        try {
            ...
            DroiLog.d("Unable to generate VastVideoConfig.", e);
            return null;
        }
    }
    ...
    VastVideoConfig evaluateVastXmlManager(@NonNull final String vastXml,
        ...
        try {
            ...
            DroiLog.d("Failed to parse VAST XML", e);
        ...
        for (VastAdXmlManager vastAdXmlManager : vastAdXmlManagers) {
            ...
            if (vastInLineXmlManager != null) {
                ...
                if (vastVideoConfig != null) {
                    populateDroiCustomElements(xmlManager, vastVideoConfig);
    ...
    private void populateVideoViewabilityTracker(
        ...
        if (vastExtensionParentXmlManager != null) {
            ...
            for (VastExtensionXmlManager vastExtensionXmlManager : vastExtensionXmlManagers) {
                if (DROI.equals(vastExtensionXmlManager.getType())) {
    ...
    private String evaluateWrapperRedirect(@NonNull VastWrapperXmlManager vastWrapperXmlManager,
        ...
        try {
            ...
            DroiLog.d("Failed to follow VAST redirect", e);
    ...
    /**
     * Parses all custom Droi specific custom extensions and impression trackers
     ...
     *                               impression trackers
     */
    private void populateDroiCustomElements(@NonNull final VastXmlManager xmlManager,
            @NonNull final VastVideoConfig vastVideoConfig) {
        Preconditions.checkNotNull(xmlManager, "xmlManager cannot be null");
        Preconditions.checkNotNull(vastVideoConfig, "vastVideoConfig cannot be null");

        vastVideoConfig.addImpressionTrackers(xmlManager.getDroiImpressionTrackers());
    ...
    /**
     * Since Droi does not support ad pods, do not accept any positive integers greater than 1.
     * Droi will use the first ad in an ad pod (sequence = 1), but it will ignore all other ads in
     * the pod. If no sequence number, Droi treats it like a stand-alone ad. If the sequence number
     * is nonsensical (e.g. negative, fails to parse as an integer), Droi treats it like a
     * stand-alone ad.
     *
     * @param sequence The sequence number
     * @return True if this is a sequence number that Droi would show an ad for, false if not.
     */
    ...
    private String followVastRedirect(@NonNull final String redirectUrl) throws IOException {
        ...
        if (mTimesFollowedVastRedirect < MAX_TIMES_TO_FOLLOW_VAST_REDIRECT) {
            ...
            try {
                urlConnection = DroiHttpUrlConnection.getHttpUrlConnection(redirectUrl);
