...
public class DroiEvents {
    ...
    static EventDispatcher getDispatcher() {
        ...
        if (result == null) {
            synchronized (DroiEvents.class) {
                result = sEventDispatcher;
                if (result == null) {
                    ArrayList<EventRecorder> recorders = new ArrayList<EventRecorder>();
                    HandlerThread handlerThread = new HandlerThread("droi_event_logging");
                    handlerThread.start();
