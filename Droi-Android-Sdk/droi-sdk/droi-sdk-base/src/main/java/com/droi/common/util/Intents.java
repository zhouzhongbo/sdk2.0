...
import com.droi.common.DroiBrowser;
...
import com.droi.exceptions.UrlParseException;
...
public class Intents {
     ...
     * an external browser, rather than the Droi in-app browser. Properly formatted native browser
     * URLs take the form of "droinativebrowser://navigate?url=https%3A%2F%2Fwww.droi.com".
     *
    ...
    public static Intent intentForNativeBrowserScheme(@NonNull final Uri uri)
        ...
        if (!UrlAction.OPEN_NATIVE_BROWSER.shouldTryHandlingUrl(uri)) {
            throw new UrlParseException("URL does not have droinativebrowser:// scheme.");
        }
        ...
        try {
            ...
            DroiLog.w("Could not handle url: " + uri);
     ...
     * "droishare://tweet?screen_name=<SCREEN_NAME>&tweet_id=<TWEET_ID>"
     *
     ...
     *
     * Example user flow:
     * Upon clicking "droishare://tweet?screen_name=SpaceX&tweet_id=596026229536460802" in an ad,
    ...
    public static Intent intentForShareTweet(@NonNull final Uri uri)
        ...
        if (!UrlAction.HANDLE_SHARE_TWEET.shouldTryHandlingUrl(uri)) {
            throw new UrlParseException("URL does not have droishare://tweet? format.");
        }
    ...
    /**
     * Launches a {@link DroiBrowser} activity with the desired URL.
     * @param context The activity context.
     * @param uri The URL to load in the started {@link DroiBrowser} activity.
     */
    public static void showDroiBrowserForUrl(@NonNull final Context context,
            @NonNull Uri uri,
        ...
        DroiLog.d("Final URI to show in browser: " + uri);

        final Bundle extras = new Bundle();
        extras.putString(DroiBrowser.DESTINATION_URL_KEY, uri.toString());
        if (!TextUtils.isEmpty(dspCreativeId)) {
            extras.putString(DroiBrowser.DSP_CREATIVE_ID, dspCreativeId);
        }
        Intent intent = getStartActivityIntent(context, DroiBrowser.class, extras);

        String errorMessage = "Could not show DroiBrowser for url: " + uri + "\n\tPerhaps you " +
                "forgot to declare com.droi.common.DroiBrowser in your Android manifest file.";
        ...
        if (deviceCanHandleIntent(context, intent)) {
            ...
            if (TextUtils.isEmpty(fallbackUrl)) {
                ...
                if (Constants.HTTP.equalsIgnoreCase(fallbackScheme)
                    ...
                    showDroiBrowserForUrl(context, fallbackUri, null);
                } else {
