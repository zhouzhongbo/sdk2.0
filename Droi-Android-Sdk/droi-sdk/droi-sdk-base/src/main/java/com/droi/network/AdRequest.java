...
import com.droi.common.FullAdType;
import com.droi.common.LocationService;
...
import com.droi.common.event.Event;
import com.droi.common.event.EventDetails;
import com.droi.common.event.DroiEvents;
import com.droi.common.logging.DroiLog;
import com.droi.common.util.Json;
import com.droi.common.util.ResponseHeader;
import com.droi.mobileads.AdTypeTranslator;
...
import static com.droi.network.HeaderUtils.extractBooleanHeader;
import static com.droi.network.HeaderUtils.extractHeader;
import static com.droi.network.HeaderUtils.extractIntegerHeader;
import static com.droi.network.HeaderUtils.extractPercentHeaderString;
...
public class AdRequest extends Request<AdResponse> {
    ...
    protected Response<AdResponse> parseNetworkResponse(final NetworkResponse networkResponse) {
        ...
        if (extractBooleanHeader(headers, ResponseHeader.WARMUP, false)) {
            return Response.error(new DroiNetworkError("Ad Unit is warming up.", DroiNetworkError.Reason.WARMING_UP));
        }
        ...
        // that it is parsed and passed along to the DroiNetworkError.
        ...
        if (AdType.CLEAR.equals(adTypeString)) {
            ...
            return Response.error(
                    new DroiNetworkError(
                            "No ads found for ad unit.",
                            DroiNetworkError.Reason.NO_FILL,
        ...
        // Used by AdViewController, Rewarded Video, Native Adapter, DroiNative
        ...
        if (AdType.STATIC_NATIVE.equals(adTypeString) || AdType.VIDEO_NATIVE.equals(adTypeString)) {
            try {
                ...
                return Response.error(
                        new DroiNetworkError("Failed to decode body JSON for native ad format",
                                e, DroiNetworkError.Reason.BAD_BODY));
            }
        }
        ...
        try {
            ...
            return Response.error(
                    new DroiNetworkError("Failed to decode server extras for custom event data.",
                            e, DroiNetworkError.Reason.BAD_HEADER_DATA));
        }
        ...
        if (eventDataIsInResponseBody(adTypeString, fullAdTypeString)) {
            // Some Droi-specific custom events get their serverExtras from the response itself:
        ...
        if (AdType.VIDEO_NATIVE.equals(adTypeString)) {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
                return Response.error(new DroiNetworkError("Native Video ads are only supported" +
                        " for Android API Level 16 (JellyBean) and above.",
                        DroiNetworkError.Reason.UNSPECIFIED));
    ...
    String getRequestId(@Nullable String failUrl) {
        ...
        try {
            ...
            DroiLog.d("Unable to obtain request id from fail url.");
        }
