...
import static com.droi.nativeads.DroiNative.DroiNativeNetworkListener;

/**
 * An ad source responsible for requesting ads from the Droi ad server.
 *
 ...
 *
 * The cache size may be automatically adjusted by the Droi server based on an app's usage and
...
class NativeAdSource {
    ...
    @NonNull private final DroiNativeNetworkListener mDroiNativeNetworkListener;
    ...
    @Nullable private DroiNative mDroiNative;
    ...
    NativeAdSource(@NonNull final List<TimestampWrapper<NativeAd>> nativeAdCache,
        ...
        mDroiNativeNetworkListener = new DroiNativeNetworkListener() {
            @Override
     ...
     */
    void registerAdRenderer(@NonNull final DroiAdRenderer droiNativeAdRenderer) {
        mAdRendererRegistry.registerAdRenderer(droiNativeAdRenderer);
        if (mDroiNative != null) {
            mDroiNative.registerAdRenderer(droiNativeAdRenderer);
        }
    }

    @Nullable
    public DroiAdRenderer getAdRendererForViewType(final int viewType) {
    ...
    void loadAds(@NonNull final Activity activity,
        ...
        loadAds(requestParameters, new DroiNative(activity, adUnitId, mDroiNativeNetworkListener));
    }

    @VisibleForTesting
    void loadAds(final RequestParameters requestParameters,
             final DroiNative droiNative) {
        clear();

        for (DroiAdRenderer renderer : mAdRendererRegistry.getRendererIterable()) {
            droiNative.registerAdRenderer(renderer);
        }

        mRequestParameters = requestParameters;
        mDroiNative = droiNative;
    ...
    @VisibleForTesting
    void replenishCache() {
        if (!mRequestInFlight && mDroiNative != null && mNativeAdCache.size() < CACHE_LIMIT) {
            mRequestInFlight = true;
            mDroiNative.makeRequest(mRequestParameters, mSequenceNumber);
        }
    }

    @Deprecated
    @VisibleForTesting
    void setDroiNative(final DroiNative droiNative) {
        mDroiNative = droiNative;
    }
