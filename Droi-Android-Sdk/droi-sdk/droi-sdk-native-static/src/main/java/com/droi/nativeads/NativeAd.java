...
import com.droi.nativeads.DroiCustomEventNative.DroiStaticNativeAd;
...
import static com.droi.nativeads.BaseNativeAd.NativeEventListener;

/**
 * This class represents a native ad instance returned from the Droi Ad Server, Droi Exchange, or
 ...
 *
 * Using {@link DroiStreamAdPlacer}, you can automatically have {@link NativeAd}s rendered into
 * {@link View}s and inserted into your app's content stream without manipulating this class
 * directly.
 *
 * In general you should get instances of {@link NativeAd} from {@link DroiNative} instances in its
 * {@link DroiNative.DroiNativeNetworkListener#onAdLoad(AdResponse)} callback
...
public class NativeAd {
    ...
    /**
     * Listen for Droi specific click and impression events
     */
    public interface DroiNativeEventListener {
    ...
    @NonNull private final DroiAdRenderer mDroiAdRenderer;
    @NonNull private final Set<String> mImpressionTrackers;
    @NonNull private final Set<String> mClickTrackers;
    @NonNull private final String mAdUnitId;
    @Nullable private DroiNativeEventListener mDroiNativeEventListener;
    ...
    public NativeAd(@NonNull final Context context,
            @NonNull final String mDroiImpressionTrackerUrl,
            @NonNull final String mDroiClickTrackerUrl,
            @NonNull final String adUnitId,
            @NonNull final BaseNativeAd baseNativeAd,
            @NonNull final DroiAdRenderer droiAdRenderer) {
        mContext = context.getApplicationContext();

        mAdUnitId = adUnitId;

        mImpressionTrackers = new HashSet<String>();
        mImpressionTrackers.add(mDroiImpressionTrackerUrl);
        mImpressionTrackers.addAll(baseNativeAd.getImpressionTrackers());

        mClickTrackers = new HashSet<String>();
        mClickTrackers.add(mDroiClickTrackerUrl);
        ...
        mDroiAdRenderer = droiAdRenderer;
    }
    ...
    public void setDroiNativeEventListener(@Nullable final DroiNativeEventListener droiNativeEventListener) {
        mDroiNativeEventListener = droiNativeEventListener;
    }
     ...
     * source. If it comes from the Droi Exchange or is a directly-served ad it will be of the type
     * {@link DroiStaticNativeAd} or DroiVideoNativeAd. If it is returned by a mediated ad
    ...
    public View createAdView(@NonNull final Context context, @Nullable final ViewGroup parent) {
        return mDroiAdRenderer.createAdView(context, parent);
    }

    public void renderAdView(View view) {
        //noinspection unchecked
        mDroiAdRenderer.renderAdView(view, mBaseNativeAd);
    }

    @NonNull
    public DroiAdRenderer getDroiAdRenderer() {
        return mDroiAdRenderer;
    }
    ...
    void recordImpression(@Nullable final View view) {
        ...
        if (mDroiNativeEventListener != null) {
            mDroiNativeEventListener.onImpression(view);
        }
    ...
    void handleClick(@Nullable final View view) {
        ...
        if (mDroiNativeEventListener != null) {
            mDroiNativeEventListener.onClick(view);
        }
