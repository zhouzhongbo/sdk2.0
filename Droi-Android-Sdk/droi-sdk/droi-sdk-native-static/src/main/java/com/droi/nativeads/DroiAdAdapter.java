...
import com.droi.common.Preconditions.NoThrow;
import com.droi.common.VisibleForTesting;
import com.droi.nativeads.DroiNativeAdPositioning.DroiClientPositioning;
import com.droi.nativeads.DroiNativeAdPositioning.DroiServerPositioning;
...
import static com.droi.nativeads.VisibilityTracker.VisibilityTrackerListener;

/**
 * {@code DroiAdAdapter} facilitates placing ads into an Android {@link android.widget.ListView} or
 ...
 *
 * This adapter uses a {@link com.droi.nativeads.DroiStreamAdPlacer} object internally. If you
 * wish to avoid wrapping your original adapter, you can use {@code DroiStreamAdPlacer} directly.
 */
public class DroiAdAdapter extends BaseAdapter {
    @NonNull private final WeakHashMap<View, Integer> mViewPositionMap;
    @NonNull private final Adapter mOriginalAdapter;
    @NonNull private final DroiStreamAdPlacer mStreamAdPlacer;
    @NonNull private final VisibilityTracker mVisibilityTracker;

    @Nullable private DroiNativeAdLoadedListener mAdLoadedListener;

    /**
     * Creates a new DroiAdAdapter object.
     *
     * By default, the adapter will contact the server to determine ad positions. If you
     * wish to hard-code positions in your app, see {@link DroiAdAdapter (Context,
     * DroiClientPositioning)}.
     *
     * @param activity The activity.
     * @param originalAdapter Your original adapter.
     */
    public DroiAdAdapter(@NonNull final Activity activity, @NonNull final Adapter originalAdapter) {
        this(activity, originalAdapter, DroiNativeAdPositioning.serverPositioning());
    }

    /**
     * Creates a new DroiAdAdapter object, using server positioning.
     *
     * @param activity The activity.
     * @param originalAdapter Your original adapter.
     * @param adPositioning A positioning object for specifying where ads will be placed in your
     * stream. See {@link DroiNativeAdPositioning#serverPositioning()}.
     */
    public DroiAdAdapter(@NonNull final Activity activity,
                         @NonNull final Adapter originalAdapter,
                         @NonNull final DroiServerPositioning adPositioning) {
        this(new DroiStreamAdPlacer(activity, adPositioning), originalAdapter,
                new VisibilityTracker(activity));
    }

    /**
     * Creates a new DroiAdAdapter object, using client positioning.
     *
     * @param activity The activity.
     * @param originalAdapter Your original adapter.
     * @param adPositioning A positioning object for specifying where ads will be placed in your
     * stream. See {@link DroiNativeAdPositioning#clientPositioning()}.
     */
    public DroiAdAdapter(@NonNull final Activity activity,
                         @NonNull final Adapter originalAdapter,
                         @NonNull final DroiClientPositioning adPositioning) {
        this(new DroiStreamAdPlacer(activity, adPositioning), originalAdapter,
                new VisibilityTracker(activity));
    }

    @VisibleForTesting
    DroiAdAdapter(@NonNull final DroiStreamAdPlacer streamAdPlacer,
                  @NonNull final Adapter originalAdapter,
        ...
        mStreamAdPlacer.setAdLoadedListener(new DroiNativeAdLoadedListener() {
            @Override
    ...
    /**
     * Registers a {@link DroiStaticNativeAdRenderer}, supporting a specific native ad format,
     ...
     */
    public final void registerAdRenderer(@NonNull final DroiAdRenderer adRenderer) {
     ...
     */
    public final void setAdLoadedListener(@Nullable final DroiNativeAdLoadedListener listener) {
        mAdLoadedListener = listener;
    }

    /**
     * Start loading ads from the Droi server.
     *
    ...
    /**
     * Start loading ads from the Droi server, using the given request targeting information.
     *
     * When loading ads, {@link DroiNativeAdLoadedListener#onAdLoaded(int)} will be called for each
     ...
     * DroiNativeAdLoadedListener} you will see a call to {@code onAdRemoved} for each of the old
     ...
     * listen for ads to load using {@link DroiNativeAdLoadedListener#onAdLoaded(int)}.
     *
     ...
     * method.
     *
     * When ads are cleared, {@link DroiNativeAdLoadedListener#onAdRemoved} will be called for each
    ...
    public int getItemViewType(final int position) {
        ...
        if (viewType != DroiStreamAdPlacer.CONTENT_VIEW_TYPE) {
     ...
     *
     * @see {@link DroiStreamAdPlacer#getOriginalPosition(int)}
     ...
     *
     * @see {@link DroiStreamAdPlacer#getAdjustedPosition(int)}
     ...
     *
     * @see {@link DroiStreamAdPlacer#insertItem(int)}
     ...
     *
     * @see {@link DroiStreamAdPlacer#removeItem(int)}
     * @param originalPosition The original content position at which to add an item. If you have an
     * adjusted position, you will need to call {@link #getOriginalPosition} to get this value.
     */
    ...
    public void setOnClickListener(@NonNull final ListView listView,
        ...
        if (!NoThrow.checkNotNull(listView, "You called DroiAdAdapter.setOnClickListener with a" +
                " null ListView")) {
            return;
        }
        if (listener == null) {
    ...
    public void setOnItemLongClickListener(@NonNull final ListView listView,
        ...
        if (!NoThrow.checkNotNull(listView, "You called DroiAdAdapter." +
    ...
    public void setOnItemSelectedListener(@NonNull final ListView listView,
        ...
        if (!NoThrow.checkNotNull(listView, "You called DroiAdAdapter.setOnItemSelectedListener" +
    ...
    public void setSelection(@NonNull final ListView listView, final int originalPosition) {
        if (!NoThrow.checkNotNull(listView, "You called DroiAdAdapter.setSelection with a null " +
                "ListView")) {
            return;
        }
    ...
    public void smoothScrollToPosition(@NonNull final ListView listView,
        ...
        if (!NoThrow.checkNotNull(listView, "You called DroiAdAdapter.smoothScrollToPosition " +
                "with a null ListView")) {
            return;
        }
    ...
    public void refreshAds(@NonNull final ListView listView,
        ...
        if (!NoThrow.checkNotNull(listView, "You called DroiAdAdapter.refreshAds with a null " +
                "ListView")) {
            return;
        }
