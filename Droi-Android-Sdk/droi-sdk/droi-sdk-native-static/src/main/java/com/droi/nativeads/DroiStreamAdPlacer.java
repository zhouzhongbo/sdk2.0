...
import com.droi.nativeads.PositioningSource.PositioningListener;
...
/**
 * {@code DroiStreamAdPlacer} facilitates loading ads and placing them into a content stream.
 *
 * If you are inserting ads into a ListView, we recommend that you use a {@link DroiAdAdapter}
 ...
 * UI thread.
 */
public class DroiStreamAdPlacer {
    /**
    ...
    private final static DroiNativeAdLoadedListener EMPTY_NATIVE_AD_LOADED_LISTENER =
            new DroiNativeAdLoadedListener() {
                @Override
    ...
    @NonNull private DroiNativeAdLoadedListener mAdLoadedListener =
    ...
    /**
     * Creates a new DroiStreamAdPlacer object.
     *
     * By default, the StreamAdPlacer will contact the server to determine ad positions. If you
     * wish to hard-code positions in your app, see {@link com.droi.nativeads.DroiStreamAdPlacer (Context,
     * DroiClientPositioning)}.
     *
     * @param activity The activity.
     */
    public DroiStreamAdPlacer(@NonNull final Activity activity) {
        // DroiClientPositioning is mutable, so we must take care not to hold a
        // reference to it that might be subsequently modified by the caller.
        this(activity, DroiNativeAdPositioning.serverPositioning());
    }

    /**
     * Creates a new DroiStreamAdPlacer object, using server positioning.
     *
     * @param activity The activity.
    ...
    /**
     * Creates a new DroiStreamAdPlacer object, using client positioning.
     *
     * @param activity The activity.
     * @param adPositioning A positioning object for specifying where ads will be placed in your
    ...
    @VisibleForTesting
    DroiStreamAdPlacer(@NonNull final Activity activity,
     ...
     */
    public void registerAdRenderer(@NonNull final DroiAdRenderer adRenderer) {
    ...
    @Nullable
    public DroiAdRenderer getAdRendererForViewType(int viewType) {
     ...
     */
    public void setAdLoadedListener(@Nullable final DroiNativeAdLoadedListener listener) {
     ...
     *
     * When loading ads, {@link DroiNativeAdLoadedListener#onAdLoaded(int)} will be called for
     * each ad that is added to the stream.
     *
     * To refresh ads in your stream, call {@code loadAds} again. When new ads load, they will
     * replace the current ads in your stream. If you are using {@code DroiNativeAdLoadedListener}
    ...
    public void loadAds(@NonNull final String adUnitId,
        ...
        if (mAdSource.getAdRendererCount() == 0) {
            DroiLog.w("You must register at least 1 ad renderer by calling registerAdRenderer " +
                    "before loading ads");
            return;
        }
        ...
        mPositioningSource.loadPositions(adUnitId, new PositioningListener() {
            @Override
            public void onLoad(@NonNull final DroiClientPositioning positioning) {
            ...
            public void onFailed() {
                ...
                DroiLog.d("Unable to show ads because ad positions could not be loaded from " +
                        "the Droi ad server.");
            }
        });
    ...
    @VisibleForTesting
    void handlePositioningLoad(@NonNull final DroiClientPositioning positioning) {
     ...
     * for ads to load using {@link DroiNativeAdLoadedListener#onAdLoaded(int)}.
     *
     ...
     *
     * This method is useful when implementing your own Adapter using {@code DroiStreamAdPlacer}.
     * To avoid worrying about view type, consider using {@link DroiAdAdapter} instead of this
     * class.
     *
     ...
     * otherwise create it. See {@link DroiAdRenderer#createAdView(Context, ViewGroup)}.
     *
     ...
     * DroiNativeAdLoadedListener#onAdLoaded (int)} each time an ad is loaded in the stream.
     *
     * @param originalCount The original number of items.
     */
