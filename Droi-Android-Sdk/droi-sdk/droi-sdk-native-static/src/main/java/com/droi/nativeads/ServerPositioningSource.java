...
/**
 * Requests positioning information from the Droi ad server.
 *
...
class ServerPositioningSource implements PositioningSource {
    ...
    private final Response.Listener<DroiClientPositioning> mPositioningListener;
    ...
    ServerPositioningSource(@NonNull final Context context) {
        ...
        mPositioningListener = new Response.Listener<DroiClientPositioning>() {
            @Override
            public void onResponse(final DroiClientPositioning clientPositioning) {
        ...
        mErrorListener = new Response.ErrorListener() {
            ...
            public void onErrorResponse(final VolleyError error) {
                ...
                if (!(error instanceof DroiNetworkError) ||
                        ((DroiNetworkError) error).getReason().equals(DroiNetworkError.Reason.WARMING_UP)) {
                    DroiLog.e("Failed to load positioning data", error);
    ...
    private void requestPositioningInternal() {
        DroiLog.d("Loading positioning from: " + mRetryUrl);
    ...
    private void handleSuccess(@NonNull DroiClientPositioning positioning) {
        if (mListener != null) {
    ...
    private void handleFailure() {
        ...
        if (delay >= mMaximumRetryTimeMillis) {
            DroiLog.d("Error downloading positioning information");
            if (mListener != null) {
                mListener.onFailed();
            }
            mListener = null;
